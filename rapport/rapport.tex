...
\paragraph*{}
Les fonction merge et split sont, contrairement à la programmation instinctive, ittératives. En effet, pour des nombres de villes suffisament grands, des fonctions itératives soulèvent des \texttt{stack overflow}.
...
\begin{algorithm}[H]
  \SetAlgoLined
  \Donnees{La chaîne de caractère $(c_i)_{i \in [0,n-1]}$ correspondant au nom de la ville recherchée et la trie contenant les données}
  \Res{Les coordonnées de cette ville ou une erreur si elle n'est pas présente}
  \eSi{La lettre $c_0$ est présente à cette étage de la trie} {
    \eSi{La chaine de caractère ne contient qu'un seul caractère} {
      \eSi{La trie est terminale à cet endroit} {
