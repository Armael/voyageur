...
START_TEST(test_join) {
  ...
  mu_assert(vector_get(t1->neighbors, t1->neighborsNb-1).p == t2,
	    "2 n'est pas le dernier voisin de 1");
  mu_assert(vector_get(t2->neighbors, t2->neighborsNb-1).p == t1,
	    "1 n'est pas le dernier voisin de 2");
}
END_TEST

int main() {
  t1 = ftree_create((Generic)1);
  t2 = ftree_create((Generic)2);
